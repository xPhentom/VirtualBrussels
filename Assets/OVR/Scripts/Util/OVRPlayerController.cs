...
public class OVRPlayerController : MonoBehaviour
{
	...
	void Start()
	{
        ...
        demoTimer = new Timer(1000);
        demoTimer.Elapsed += (s, ev) =>
        {
            elapsedTime++;
        };
	...
	protected virtual void Update()
	{
        ...
        if (CheckCloseToTag("BXL_ART", 4))
        {
            if (!interests.Contains("BXL_ART"))
            {
                Debug.Log("CLOSE TO ART !");
                Timer t = new Timer(3500);
                t.Elapsed += (s, ev) =>
                {
                    interests.Add("BXL_ART");
                    t.Stop();
                };
                t.Start();
            }
        ...
        if(CheckCloseToTag("BXL_BEER", 2))
        {
            if(!interests.Contains("BXL_BEER"))
            {
                Debug.Log("CLOSE TO BEER !");
                Timer t = new Timer(3500);
                t.Elapsed += (s, ev) =>
                {
                    interests.Add("BXL_BEER");
                    t.Stop();
                };
                t.Start();
            }
        ...
        if (CheckCloseToTag("BXL_COMICS", 2))
        {
            if (!interests.Contains("BXL_COMICS"))
            {
                Debug.Log("CLOSE TO COMICS !");
                Timer t = new Timer(3500);
                t.Elapsed += (s, ev) =>
                {
                    interests.Add("BXL_COMICS");
                    t.Stop();
                };
                t.Start();
            }
        ...
        if (CheckCloseToTag("BXL_FOOD", 2))
        {
            if (!interests.Contains("BXL_FOOD"))
            {
                Debug.Log("CLOSE TO FOOD !");
                Timer t = new Timer(3500);
                t.Elapsed += (s, ev) =>
                {
                    interests.Add("BXL_FOOD");
                    t.Stop();
                };
                t.Start();
            }
        ...
        if (CheckCloseToTag("BXL_MUSIC", 2))
        {
            if (!interests.Contains("BXL_MUSIC"))
            {
                Debug.Log("CLOSE TO MUSIC !");
                Timer t = new Timer(3500);
                t.Elapsed += (s, ev) =>
                {
                    interests.Add("BXL_MUSIC");
                    t.Stop();
                };
                t.Start();
            }
        ...
        if(elapsedTime >= 20 && controlsEnabled == true)
        {
            interests = interests.Distinct().ToList();
            uitext.text = "Timeout ! Je interesses zijn : \n";
            Debug.Log(interests.Count);
            foreach(string inter in interests)
            {
                uitext.text += "\n- " + inter;
            }
            uitext.text += "\n \n Wij raden je volgende plaatsen aan in Brussel :";
            uitext.text += "\n - Restaurant X";
            uitext.text += "\n - Museum Y";
            uitext.text += "\n - Plaats Z";
    ...
    public void TriggerInterest(string interestName, double timeUntilInterested)
    {
    ...
    /// <summary>
	/// Verify if the player is close to a gameObject.
	/// </summary>
    bool CheckCloseToTag(string tag, float minimumDistance)
    {
        ...
        for (int i = 0; i < goWithTag.Length; ++i)
        {
            ...
            //Debug.Log(Vector3.Distance(transform.position, goWithTag[i].transform.position));
        }
